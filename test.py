...
def parepareDataset(device):
    ...
    _test_set = Flickr8kDataset(dataset_base_path=DATASET_BASE_PATH, dist='test', device=device,
                            return_type='tensor',
                            load_img_to_memory=False)
    ...
    _test_set.transformations = eval_transformations

    vocab_size = len(vocab)
    print("Dataset Preparation Complete")

    return test_set, _test_set, word2idx, idx2word, vocab_size


def prepareLoader(test_set, _test_set, vocab_size):
    ...
    _test_loader = DataLoader(_test_set, batch_size=BATCH_SIZE, shuffle=False, sampler=None, pin_memory=False)

    print("Dataloader Preparation Complete")

    return test_loader, _test_loader


def evaluate_model(data_loader, _data_loader, model, loss_fn, word2idx, vocab_size, bleu_score_fn, meteor_score_fn, tensor_to_word_fn, desc=''):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(t):
            ...
            if _data_loader is not None:
                _batch = next(iter(_data_loader));
                _images, _captions, _lengths = _batch
                scores, caps_sorted, decode_lengths, alphas, sort_ind = model(_images, _captions, _lengths)
    ...
    if _data_loader is not None:
        return running_loss, running_acc, running_bleu, running_meteor
    else:
...
def test():
    ...
    test_set, _test_set, word2idx, idx2word, vocab_size = parepareDataset(device)
    test_loader, _test_loader = prepareLoader(test_set, _test_set, vocab_size)
    # Word embedding
